...
void desc();
int flag = 1, cou=0;
GLfloat dx = 0, dy = 0, tx = 0, ty = 0, mx = 0, my = 0, rx = 0, ry = 0,xx,yy,ax;
GLfloat a, b;
...
void body()
{
	...
	//Right hand
	glBegin(GL_POLYGON);
		glVertex3f(12, 15.4, 0);
		glVertex3f(6.7, 15.4, 0);
...
void digest()
{
	...
	digesdraw();
	glPushMatrix();
	glTranslatef(dx + tx, dy + ty, 0.0);//mouth to pharynx
	food();
	glPopMatrix();
	glPushMatrix();
	glTranslatef(dx + mx, dy + my, 0.0);//pharynx to liver
	foodp();
	glPopMatrix();
	glPushMatrix();
	glTranslatef(dx + tx, dy + ty, 0.0);//liver to stomach
	food();
	glPopMatrix();
	liver();
	glPushMatrix();
	glTranslatef(-1.7 , -33.5 + yy, 0.0);//small
	foods();
	glPopMatrix();
	largeinstes();
	if (countd == 1)
	...
	if (countd == 0)
	{
		glColor3ub(115, 183, 0);
		glLineWidth(3);
		drawline(a - 9, b - 36, a - 9.5, b - 37);//arrow
		drawline(a - 9, b - 36, a - 9.5, b - 35);//arrow
		drawline(a - 23, b - 35, a - 9, b - 36);
		drawstring(a -30, b -35, 0, "bile juice");
		countd = 3;
...
void animate()
{
	if (flag == 1)
	{
		if (tx < 20 && ty < 20 && theta <= 3.14)//mouth
		{
		...
		if (theta >= 3.14 && ty >= -20.5)//esophagus
		{
			ty = ty - 0.01;
			countd = 3;
		}

		if (theta >= 3.14 && theta1 >= 3 * 3.14 / 2.0 && ty <= -20.5 && ty >= -30)//liver
		{
			mx = -1.5 + cos(theta1) * 4;
			my = -4.8 + sin(theta1) * 4;
			theta1 -= 0.005;
		...
		else if (theta1 <= 3 * 3.14 / 2.0 && my <= -5.79 && my >= -11 && theta2 <= 3.0)//stomach
		{
			mx = mx - 0.01;
			my = my - 0.01;
			countd = 3;
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("Human Digestive and Respiratory");
	init();
	...
	glutKeyboardFunc(myKeyboardFunc);
	PlaySound(TEXT("ChillingMusic.wav"), NULL, SND_ASYNC | SND_FILENAME | SND_LOOP);
	glutMainLoop();
	return 0;
}